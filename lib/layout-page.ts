...
export const leaguePageClass =
  "data-app-page mx-auto min-h-screen w-full max-w-lg px-4 pb-[calc(4.75rem+env(safe-area-inset-bottom))] pt-[calc(env(safe-area-inset-top)+4rem)] md:mx-0 md:max-w-none md:px-0 md:pl-56 md:pt-8 md:pb-8 md:flex md:justify-center";

/** Внутренний блок — по центру оставшейся ширины на десктопе. */
export const appPageContentClass = "w-full max-w-2xl px-0 md:px-16";

/** Контент страниц лиги (горизонтальные отступы задаёт layout). */
export const leagueMainClass = "w-full pt-6 pb-8";
